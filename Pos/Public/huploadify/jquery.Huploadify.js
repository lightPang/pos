				  ...
				  for(var i=0,len=files.length;i<len;i++){
				  	var thisFile = files[i];
				  		if(parseInt(formatFileSize(thisFile.size,true))>option.fileSizeLimit){
				  			alert('文件'+thisFile.name+'大小超出限制，最大不超过'+option.fileSizeLimit+"KB!");
				  			continue;
				  		}
				  		if( thisFile.name.indexOf('_') >=0 || thisFile.name.indexOf(',') >= 0){
				  			alert("上传文件中请不要包含 '_' 或 ',' 字符！");
				  			continue;
				  		}
